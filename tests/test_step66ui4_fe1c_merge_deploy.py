"""Step 66UI.4-FE.1C-MD -- merge PR #10 and calibrate test runtime (docs-only checks).

This file changes no runtime code. It confirms the merge record and merged-main test deployment
record exist and state the required facts: PR #10 merge, Product Owner merge authorization,
Product Owner UI validation VISIBLE, presence of the review/live-verification/preview-deployment/
validation artifacts on main, that the deployment source is merged main, test-runtime-only scope,
the accepted non-blocking TaskList query-param gap, no backend/API/database/workflow/new-endpoint
change, no production/external action, FE.1D remaining unauthorized,
production_executed_true_count remaining 0, Local Artifact Reconciliation, and the secret-scan
informational GUID note.
"""
...
def test_progress_references_stage() -> None:
    ...
    assert "66ui.4-fe.1c-md" in text


def test_pr10_commit_and_merge_recorded() -> None:
...
def test_product_owner_validation_visible_recorded() -> None:
    text = _norm(_all_text())
...
def test_all_stage_artifacts_referenced() -> None:
    text = _norm(_all_text())
    for path in (
        "step66ui4-fe1c-implementation-review-record",
        "step66ui4-fe1c-live-agent-execution-verification-record",
        "step66ui4-fe1c-ui-validation-preview-deployment-record",
        "step66ui4-fe1c-product-owner-validation",
    ):
...
def test_tasklist_query_param_gap_accepted_non_blocking() -> None:
    text = _norm(_all_text())
    assert "tasklist" in text
    assert "query-param" in text
    assert "non-blocking" in text or "accepted" in text
...
def test_secret_scan_informational_guid_note_recorded() -> None:
    text = _norm(_all_text())
    assert "informational=100" in text or "informational count is 100" in text
    assert "guid" in text
...
def test_marker_pass_present() -> None:
    text = DOCS["fe1c-merged-main-deploy-record"].read_text(encoding="utf-8")
    assert "STEP66UI4_FE1C_MERGE_DEPLOY_VERIFY: PASS" in text
