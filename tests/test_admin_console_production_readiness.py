"""Step 62 -- Admin Console production readiness view (read-only, no action control)."""
...
def test_route_and_nav_registered() -> None:
    assert "/production-readiness" in APP.read_text(encoding="utf-8")
    assert "/production-readiness" in NAV.read_text(encoding="utf-8")


def test_operations_getters_present() -> None:
    ops = OPS.read_text(encoding="utf-8")
    assert "getReadinessPolicy" in ops
    assert "/operations/readiness/" in ops
