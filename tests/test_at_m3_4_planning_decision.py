...
Rebaselined after AT-M3.4 Validation 2, which failed on a different axis entirely: a
ReasoningInvocation could reach 'succeeded' durably while the artifact it produced existed only in
the memory of a process that then died, leaving the correlation terminal, the plan unrecoverable
and the converged discussion permanently stranded. Two groups below defend the fix -- the crash
matrix, which kills the runtime at each commit boundary and asserts recovery WITHOUT a second
provider call, and the provenance group, which now requires the planner to be a seated participant
of the discussion rather than anyone the router could reach afterwards.
"""
...
async def _skip_without_migration() -> None:
    ...
    try:
        if await conn.fetchval("SELECT to_regclass('public.planning_decisions')") is None:
            pytest.skip("migration 041 not applied; skipping AT-M3.4 store test")
        if not await conn.fetchval(
            ...
            pytest.skip("the ledger predates the candidate-plan binding; skipping")
        # 041 without 040 would be a database where the plan can be bound but not recovered.
        if not await conn.fetchval(
...
async def test_two_planless_discussions_conflict_rather_than_replay():
    ...
    for session in (first, second):
        assert (await DeliberationStore().get_session(str(session["discussion_id"])))[
            "state"
        ] == "converged"
...
# --- K. actor provenance: the planner was in the room ---------------------------------------


@pytest.mark.asyncio
async def test_the_plan_is_attributed_to_the_seated_planner_and_to_nobody_else():
    scenario = await _scenario()
    session = await _converged(scenario)
    result = await _service().finalize(
        goal_id=scenario["goal_id"], discussion_id=str(session["discussion_id"])
...
@pytest.mark.asyncio
async def test_a_project_member_who_was_not_in_the_room_can_never_become_the_author():
    """The removed fallback, asserted as removed.

    The earlier implementation, finding no seated planner, routed one from CURRENT project
    membership -- which meant a principal who had never seen the discussion could be recorded as
    the author of the plan that discussion produced. Nobody reading the result afterwards could
    tell that apart from a plan its author had argued for.

    Agent A is seated and cannot plan. Agent B can plan and is not seated. The correct outcome is
    that this discussion produces no plan at all.
    """
    ...
    discussion_id = str(final["session"]["discussion_id"])

    # A planner joins the PROJECT after the room has already converged. It is reachable by the
    # capability router, and that must not make it this discussion's author.
    await TeamService().form_team(
        scenario["project_id"], goal_ref="late", agent_keys=("project-planner-agent",)
    )
    latecomer = await TeamStore().routing_candidates(scenario["project_id"])
    assert any("plan_project" in c.capabilities for c in latecomer), (
        "the fixture must actually offer a routable planner, or this test proves nothing"
    )

    with pytest.raises(PlannerUnavailableError):
        await _service().finalize(goal_id=scenario["goal_id"], discussion_id=discussion_id)

    counts = await _counts(scenario["project_id"])
    assert counts["ledger"] == 0 and counts["decisions"] == 0
    assert await _candidates(scenario["project_id"], discussion_id) == []
    # And no reasoning happened: the gate is before the provider, not after it.
    assert await _invocations(discussion_id, final["session"]["result_message_id"]) == 0


@pytest.mark.asyncio
async def test_several_seated_planners_resolve_deterministically_to_the_lowest_seat():
    """Arbitrary among equals is fine; arbitrary DIFFERENTLY on a replay is not -- the plan would
    change authors between two reads of the same decision."""
    scenario = await _scenario()
    session = await _converged(scenario)
    discussion_id = str(session["discussion_id"])

    participants = await DeliberationStore().list_participants(discussion_id)
    eligible = sorted(
        (p for p in participants if "plan_project" in list(p["matched_capabilities"] or [])),
        key=lambda p: p.get("seat_index") or 0,
    )
    result = await _service().finalize(goal_id=scenario["goal_id"], discussion_id=discussion_id)
    assert str(result["team_decision"]["proposed_by"]) == str(eligible[0]["principal_id"])

    # Resolving again returns the same principal, not merely a valid one.
    again = await _service().finalize(goal_id=scenario["goal_id"], discussion_id=discussion_id)
    assert again["created"] is False
    assert str(again["team_decision"]["proposed_by"]) == str(eligible[0]["principal_id"])


# --- K2. the crash matrix: what Validation 2 failed on ---------------------------------------


async def _reasoning_row(discussion_id: str, result_message_id) -> dict:
    conn = await PlanningDecisionStore()._connect()
    try:
        row = await conn.fetchrow(
            "SELECT status, artifact, attempt, artifact_type FROM reasoning_invocations "
...
@pytest.mark.asyncio
async def test_c4_a_succeeded_invocation_whose_candidate_write_was_lost_recovers_without_recall():
    """**The load-bearing acceptance test of the rebaseline.**

    The exact failed window: the provider succeeds, the invocation commits SUCCEEDED with its
    artifact, and the process dies BEFORE the candidate TeamMessage is written. Under the old
    contract this was terminal and empty -- the correlation could never be re-invoked and replay
    returned nothing, so the converged discussion was stranded forever.

    A fresh service must now recover the EXACT artifact, write the candidate, and finalize, with
    ...
    class _DiesAfterReasoning(PlanningDecisionService):
        async def _ensure_candidate(self, **kwargs):
            # Let the reasoning call run and commit exactly as it normally would...
            await self._author_plan(**kwargs)
            # ...then die before the candidate message exists.
            raise Injected("process died between the reasoning commit and the candidate write")

    with pytest.raises(Injected):
        await _DiesAfterReasoning().finalize(
            goal_id=scenario["goal_id"], discussion_id=discussion_id
        )

    assert await _invocations(discussion_id, session["result_message_id"]) == 1
    assert await _candidates(scenario["project_id"], discussion_id) == [], (
        "the candidate message is genuinely absent -- this is the failed window"
    )

    stranded = await _reasoning_row(discussion_id, session["result_message_id"])
    assert stranded["status"] == "succeeded"
    assert stranded["artifact"] is not None, "SUCCEEDED must carry its artifact"
    assert stranded["artifact_type"] == "PlanDraftArtifact"
    artifact = json.loads(stranded["artifact"])

    # A completely fresh service, as a new process would be.
    result = await _service().finalize(goal_id=scenario["goal_id"], discussion_id=discussion_id)

    assert result["created"] is True
    assert result["outcome"] == PLAN_ACCEPTED
    candidates = await _candidates(scenario["project_id"], discussion_id)
    assert len(candidates) == 1, "exactly one candidate, recovered rather than re-derived"
    assert candidates[0]["content"] == artifact, "the EXACT artifact, byte for byte"
    assert result["plan_revision"]["plan"] == artifact["plan"]
    # The whole point: no second provider call, and no second attempt.
    assert await _invocations(discussion_id, session["result_message_id"]) == 1
    after = await _reasoning_row(discussion_id, session["result_message_id"])
    assert after["attempt"] == 1, "recovery is a replay, not a retry"
    assert json.loads(after["artifact"]) == artifact, "a terminal artifact is never rewritten"


@pytest.mark.asyncio
async def test_c5_a_candidate_that_survived_a_lost_decision_is_reused_not_reauthored():
    """The candidate commits, the decision transaction is then lost. The retry must adopt the
    candidate already on record rather than author a second one."""
    scenario = await _scenario()
    session = await _converged(scenario)
    discussion_id = str(session["discussion_id"])

    class _DiesAfterCandidate(PlanningDecisionService):
        async def _ensure_candidate(self, **kwargs):
            candidate = await super()._ensure_candidate(**kwargs)
            raise Injected(f"process died after candidate {candidate['message_id']}")

    with pytest.raises(Injected):
        await _DiesAfterCandidate().finalize(
            goal_id=scenario["goal_id"], discussion_id=discussion_id
        )

    first = await _candidates(scenario["project_id"], discussion_id)
    assert len(first) == 1
    counts = await _counts(scenario["project_id"])
    assert counts["ledger"] == 0 and counts["decisions"] == 0

    result = await _service().finalize(goal_id=scenario["goal_id"], discussion_id=discussion_id)
    again = await _candidates(scenario["project_id"], discussion_id)
    assert len(again) == 1, "the surviving candidate is reused"
    assert str(again[0]["message_id"]) == str(first[0]["message_id"])
    assert str(result["candidate_plan_message_id"]) == str(first[0]["message_id"])
    assert await _invocations(discussion_id, session["result_message_id"]) == 1


@pytest.mark.asyncio
async def test_a_provider_that_refuses_produces_no_plan_and_no_decision():
    """Fail closed. A refused provider never yields a substitute plan -- and after the rebaseline
    the refusal is durable, so a retry replays the refusal instead of quietly trying again."""
    from shared.sdk.agent_reasoning.provider import DisabledReasoningProvider

    scenario = await _scenario()
    session = await _converged(scenario)
    discussion_id = str(session["discussion_id"])
    service = PlanningDecisionService(provider=DisabledReasoningProvider())

    with pytest.raises(PlanningDecisionStateError):
        await service.finalize(goal_id=scenario["goal_id"], discussion_id=discussion_id)

    assert await _candidates(scenario["project_id"], discussion_id) == []
    counts = await _counts(scenario["project_id"])
    assert counts["ledger"] == 0 and counts["decisions"] == 0

    # The failure is terminal and recorded; retrying replays it rather than fabricating a plan.
    with pytest.raises(PlanningDecisionStateError):
        await _service().finalize(goal_id=scenario["goal_id"], discussion_id=discussion_id)
    assert await _invocations(discussion_id, session["result_message_id"]) == 1
    ...
    try:
        for column, value in (
            ...
            with pytest.raises(asyncpg.PostgresError):
                await conn.execute(
                    f"UPDATE planning_decisions SET {column}={value} WHERE planning_decision_id=$1",
                    decision_id,
                )
    finally:
        await conn.close()
...
async def test_database_wide_invariants_hold():
    ...
    try:
        checks = {
            ...
            # --- the rebaseline's own invariants ---------------------------------------------
            # Scoped to decompose_plan because that verb did not exist on canonical main, so
            # every row it has was written under the durable-artifact contract and none of them
            # can be legacy. A blanket check would be asserting something about AT-M3.1 history
            # that this migration deliberately did not rewrite.
            "a succeeded plan invocation with no artifact": (
                "SELECT count(*) FROM reasoning_invocations "
                "WHERE reasoning_verb='decompose_plan' AND status='succeeded' "
                "AND (artifact IS NULL OR artifact_type IS NULL)"
            ),
            "a non-succeeded invocation carrying an artifact": (
                "SELECT count(*) FROM reasoning_invocations "
                "WHERE status <> 'succeeded' AND artifact IS NOT NULL"
            ),
            "an artifact_type disagreeing with its verb": (
                "SELECT count(*) FROM reasoning_invocations "
                "WHERE artifact_type IS NOT NULL AND artifact_type <> CASE reasoning_verb "
                "WHEN 'propose' THEN 'ProposalArtifact' "
                "WHEN 'critique' THEN 'CritiqueArtifact' "
                "WHEN 'summarize_decision' THEN 'DecisionSummaryArtifact' "
                "WHEN 'decompose_plan' THEN 'PlanDraftArtifact' END"
            ),
            # THE equality the whole rebaseline rests on: what the team can see is what the
            # provider actually produced, with nothing substituted in between.
            "a candidate message whose content differs from its invocation artifact": (
                "SELECT count(*) FROM team_messages m "
                "JOIN reasoning_invocations ri "
                "ON ri.invocation_id = (m.artifact_refs->>'reasoning_invocation_id')::uuid "
                "WHERE m.artifact_refs ? 'candidate_plan_for_discussion' "
                "AND m.content IS DISTINCT FROM ri.artifact"
            ),
            "a terminal invocation still holding a lease": (
                "SELECT count(*) FROM reasoning_invocations "
                "WHERE status <> 'started' AND lease_expires_at IS NOT NULL"
            ),
            "an invocation recording fewer than one attempt": (
                "SELECT count(*) FROM reasoning_invocations WHERE attempt < 1"
            ),
...
def test_admissibility_names_the_clause_that_failed():
    assert (
        evaluate_admissibility(discussion=None, goal_id="g", current_plan_revision_id=None).clause
        == "exists"
    )
