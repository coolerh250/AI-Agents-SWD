...
-- questions/answers as specific message_type values) and
-- operator_clarification_requests (the clarification lifecycle: open ->
-- answered/expired/canceled). Named "operator_clarification_requests" (not
-- "clarification_requests") to avoid colliding with the pre-existing,
-- differently-shaped `clarification_requests` table from the Discord-driven
-- requirement-agent pipeline (007_flexible_task_execution_loop.sql,
-- pk=clarification_id, task_id TEXT) -- a distinct, unrelated subsystem. This
-- follows the operator_tasks / operator_action_requests naming family.
--
...
CREATE TABLE IF NOT EXISTS operator_clarification_requests (
    id                    UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    task_id               UUID NOT NULL REFERENCES operator_tasks(id),
    ...
    created_at            TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at            TIMESTAMPTZ NOT NULL DEFAULT now(),
    CONSTRAINT chk_operator_clarification_requests_status CHECK (status IN (
        'open', 'answered', 'expired', 'canceled'
    )),
    CONSTRAINT chk_operator_clarification_requests_question_nonempty
        CHECK (length(btrim(question)) > 0),
    CONSTRAINT chk_operator_clarification_requests_question_length CHECK (length(question) <= 4000)
);

CREATE INDEX IF NOT EXISTS idx_operator_clarification_requests_task_id
    ON operator_clarification_requests (task_id);
CREATE INDEX IF NOT EXISTS idx_operator_clarification_requests_status
    ON operator_clarification_requests (status);
