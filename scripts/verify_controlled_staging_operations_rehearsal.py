#!/usr/bin/env python3
"""Step 64F.2 -- Controlled staging operations rehearsal verifier.

Confirms the rehearsal docs exist and record an orchestrator-only restart + validation of the
staging runtime, with the forbidden actions (rebuild / full-stack restart / down / down -v /
teardown / restore / workflow re-run / production action) documented as NOT executed, safety
preserved (production_executed stays 0), Step 64E PASS and Step 64F REHEARSAL_COMPLETED.

Marker: CONTROLLED_STAGING_OPERATIONS_REHEARSAL_VERIFY: PASS | PASS_WITH_GAPS | FAIL
...
MARKER = "CONTROLLED_STAGING_OPERATIONS_REHEARSAL_VERIFY"

DOCS = {
    "report": REPORT,
    "before-after-evidence": EVIDENCE,
    "operator-checklist-result": CHECKLIST,
    "known-gaps": GAPS,
...
def main() -> int:
    ...
    # Orchestrator-only restart is the documented action.
    if "orchestrator-only restart" not in low:
        bad("docs do not document an orchestrator-only restart")
    if "restart orchestrator" not in report_low:
        bad("report does not record the exact `restart orchestrator` command")
    ...
    # Validation recorded: reachability + safety.
    ...
    if "rehearsal_completed" not in low:
        bad("docs do not record Step 64F REHEARSAL_COMPLETED")
    ...
    print("  [OK] orchestrator-only restart rehearsal recorded; forbidden actions not executed;")
    print("       health/admin/safety validated, no data loss, prod_exec=0; Step 64E PASS,")
    print("       Step 64F REHEARSAL_COMPLETED")
