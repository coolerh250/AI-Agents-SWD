...
def is_candidate_for(message: dict[str, Any], discussion_id: Any) -> bool:
    ...
    if not isinstance(refs, dict):
        return False
    return message.get("message_type") == PROPOSAL_MESSAGE_TYPE and str(
        refs.get(CANDIDATE_REF_KEY) or ""
    ) == str(discussion_id)
...
def derive_case(*, current_revision: dict[str, Any] | None, candidate_plan: dict[str, Any]) -> str:
