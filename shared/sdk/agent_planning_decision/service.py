...
* **No caller-supplied plan.** AT-M3.4 Validation 1 handed this slice a plan reading "REWRITE
  ...
  substitute.
* **No caller-supplied author.** The planner is resolved from the discussion's own participants and
  the AT-M2 capability router, and that principal -- the one that actually did the reasoning -- is
  who the TeamDecision and the PlanRevision are attributed to. A transport caller is not an author.
...
class PlanningDecisionService:
    ...
    async def _resolve_planner(self, discussion: dict[str, Any]) -> dict[str, Any]:
        """Who authors the plan. Resolved from the runtime team, never named by the caller.

        Preference order, and the order is the point:

        1. a participant of THIS discussion that the router already selected for ``plan_project``.
           They were in the room, so attributing the plan to them is true.
        2. otherwise the AT-M2 capability router against the project's current membership -- the
           same router that decides who takes a work item. A discussion that did not require
           planning capability can still produce a plan, but its author is chosen by the team's own
           routing rule rather than by whoever called the endpoint.

        Fail closed if neither yields one. A plan attributed to a principal that did not author it
        ...
        candidates = await self.team.routing_candidates(str(discussion["project_id"]))
        decision = route(
            RoutingRequest(
                requested_capability=PLANNER_CAPABILITY,
        ...
        if not decision.selected or decision.selected_principal_id is None:
            raise PlannerUnavailableError(
                f"no principal on project {discussion['project_id']} can author a plan "
                f"({PLANNER_CAPABILITY}): {decision.reason}"
            )
        return {
            "principal_id": str(decision.selected_principal_id),
    ...
    async def _ensure_candidate(
        ...
        **The discussion row is locked for the whole of this.** That is what makes "one candidate
        per discussion" true under concurrency without a polling loop and without a lock registry:
        eight workers queue on the row they are all consuming, the first authors the plan, and the
        other seven find it already written. AT-M3.1's correlation claim is the second, independent
        guarantee -- the correlation id is derived from the discussion, so
        ``uq_reasoning_invocations_correlation`` makes "one decompose_plan invocation per
        discussion" a database fact rather than a convention.

        The reasoning call happens inside that lock, which is safe here and would NOT be for a live
        model: under the current authorization the provider is in-process, deterministic and
        mock-only (AT-D14 section 3 -- M3.6B is not authorized), so the lock is held for
        microseconds. A slice that wires a real provider must move authorship out of this lock; the
        alternative is a database lock held open across a network call.
    ...
    async def _author_plan(
        ...
        *,
        ...
        if result.artifact is None:
            # Fail closed, and say which kind of failure it was. A refused or unavailable provider
            # never yields a substitute plan -- that is AT-D14 section 4's first safety invariant,
            # and a fabricated plan would be exactly the substitution this milestone removed.
        ...
        return {"artifact": result.artifact, "invocation_id": result.invocation["invocation_id"]}
    ...
    async def get_evidence(self, planning_decision_id: str) -> dict[str, Any] | None:
        ...
        if candidate is not None and not (
            ...
            and str((candidate.get("artifact_refs") or {}).get("goal_id"))
            == str(ledger["goal_id"])
