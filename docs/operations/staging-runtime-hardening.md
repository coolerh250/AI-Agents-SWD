...
## Secrets integration (Stage 26)

The staging compose ships `SECRET_PROVIDER`, `VAULT_ADDR`,
`VAULT_TOKEN`, `VAULT_KV_MOUNT`, `VAULT_KV_PATH`, and
`MOCK_VAULT_SECRETS_FILE` on every secret-aware service. The default
provider is `mock-vault`, which reads
`infra/runtime/.mock-vault-secrets.local.json` (generated by
`scripts/bootstrap_mock_vault_secrets.sh`, chmod `600`, gitignored).

```
./scripts/bootstrap_mock_vault_secrets.sh
SECRET_PROVIDER=mock-vault ./scripts/start_staging_runtime.sh
./scripts/verify_staging_secrets.sh
```

The full procedure (rotation smoke, leak scan, production-check
rejection of mock-vault) lives in
